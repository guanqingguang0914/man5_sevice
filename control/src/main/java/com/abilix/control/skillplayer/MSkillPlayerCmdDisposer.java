...
public class MSkillPlayerCmdDisposer extends AbstractSkillPlayerCmdDisposer {
    private static final String TAG = "MSkillPlayerCmdDisposer";

    public MSkillPlayerCmdDisposer(Handler mHandler) {
        super(mHandler);
    }
    ...
    public void disposeSkillPlayerCmd(byte[] Cmd, int modeState) {
        LogMgr.d(TAG, "M_disposeSkillPlayerCmd()");
        try {
            LogMgr.d(TAG, "SkillPlayerCmd = " + Utils.bytesToString(Cmd));
            if (modeState == 0) {
            ...
            if (playType == GlobalConfig.PLAY_TYPE_ACTION) {
                //只播放动作
                LogMgr.v(TAG, "M_DisposeProtocol 只播放动作 actionfilePath = " + actionFilePath);
                ...
                //只播放声音
                LogMgr.v(TAG, "M_DisposeProtocol 只播放声音 soundfilePath = " + soundFilePath);
...
//				String soundFilePath = filePath.replace(".bin", ".wav");
                LogMgr.v(TAG, "M_DisposeProtocol动作声音同时播放 actionfilePath = " + actionFilePath + " soundFilePath = " + soundFilePath);
                ...
                //只播视频文件
                LogMgr.v(TAG, "M_DisposeProtocol只播视频文件 soundfilePath = " + soundFilePath);
                PlayMoveOrSoundUtils.getInstance().handlePlayCmd(null, soundFilePath, loopCtr, isSkillPlayCmd, delayTime, false, PlayMoveOrSoundUtils.PLAY_MODE_NORMAL, false, true, null);
...
//				String videoFilePath = filePath.replace(".bin", ".avi");
                ...
                //播放类型错误
                LogMgr.e(TAG, "M_DisposeProtocol 播放类型错误");
                return;
            }
        } catch (Exception e) {
            e.printStackTrace();
            LogMgr.e(TAG, "disposeSkillPlayerCmd error::" + e);
        }
