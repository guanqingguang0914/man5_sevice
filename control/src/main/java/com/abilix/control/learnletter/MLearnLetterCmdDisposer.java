...
public class MLearnLetterCmdDisposer extends AbstractLearnLetterCmdDisposer {

    public MLearnLetterCmdDisposer(Handler mHandler) {
        super(mHandler);
    }
    ...
    public void disposeLearnLetterCmd(int modeState, String fileFullPath) {
        ...
        if (modeState == AbstractLearnLetterCmdDisposer.MODE_STATE_STOP  || modeState == AbstractLearnLetterCmdDisposer.MODE_STATE_EXPLAIN_STOP) {
            //停止当前动作
            LogMgr.d("modeState == 0 停止当前动作");
            ...
            //播放当前动作
            LogMgr.d("播放当前动作 modeState = "+modeState);
            ...
            if (actionFile.exists() && soundFile.exists()) {
                PlayMoveOrSoundUtils.getInstance().handlePlayCmd(actionFilePath, soundFilePath, isLoop, false, 0, false, PlayMoveOrSoundUtils.PLAY_MODE_NORMAL, false, true, zhaohui_callback);
            } else if (actionFile.exists()) {
                PlayMoveOrSoundUtils.getInstance().handlePlayCmd(actionFilePath, null, isLoop, false, 0, false, PlayMoveOrSoundUtils.PLAY_MODE_NORMAL, false, true, zhaohui_callback);
            } else {
    ...
    private PlayMoveOrSoundUtils.PlayCallBack playCallBack  = new PlayMoveOrSoundUtils.PlayCallBack() {
        @Override
        public void onStart() {
