...
public class MAVLinkStats /* implements Serializable */ {
    ...
    public void newPacket(MAVLinkPacket packet) {
        ...
        if (hasLostPackets(packet)) {//是否丢帧
            updateLostPacketCount(packet);//丢帧了
        }
    ...
    private void updateLostPacketCount(MAVLinkPacket packet) {
        int lostPackets;//丢失的帧数
        if (packet.seq - lastPacketSeq < 0) {//已经判断 != 了，那只有 > 或者 <
            lostPackets = (packet.seq - lastPacketSeq) + 255;//被相加后超过255高位溢出等于是加负数
        } else {
            lostPackets = (packet.seq - lastPacketSeq);//丢帧了的数目
        }
